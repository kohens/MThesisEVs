...
end_time = end
[forecast, measurement] = PPO.get_forecast_measurement(file_path)
times = np.arange(0, len(forecast)*0.25, 0.25)
forecast = PPO.create_time_function(times, forecast)
pv_generation = PPO.create_time_function(times, measurement)

file_path = 'Day_aheadprices.xlsx'
lamda_belpex_forecast = PPO.get_price_forecast(file_path)

price_forecast = 1.21*(45*np.ones(len(lamda_belpex_forecast))+lamda_belpex_forecast)
times = np.arange(0, len(price_forecast), 1)
price_forecast = PPO.create_time_function(times, price_forecast)
lamda_belpex = lamda_belpex_forecast

consumption_prices = 1.21*(45*np.ones(len(times))+lamda_belpex)

consumption_price = PPO.create_time_function(times, consumption_prices)
injection_price = PPO.create_time_function(times, np.ones(len(times))*lamda_inj)
...
for N_ev in N_evList:
    ...
    for L_c in L_cList:
        ...
        forecast = forecast(moments)
        price_forecast = price_forecast(moments)
        for kappa in kappaList:
            print('check')
            # Create environment and train the network
            file_name = 'N_ev_'+str(N_ev)+'_L_c_'+str(L_c)+'_kappa_'+str(kappa)+'.npz'
            np.savez(file_name,array1=T_arr, array2=T_dep, array3=E_reqs, array4=forecast, array5=price_forecast, array6=moments, array7=pv_generation(moments), array8=consumption_price(moments))
            env = PPO.Environment(start_time, int(end_time+1), N_ev, forecast, price_forecast, N_past, N_fut, L_c, E_reqs, P_max, T_arr, T_dep, Load_Stations, StationVector, IDs, kappa ,injection_price,consumption_price, pv_generation )
            agent = PPO.PPOAgent(state_dim, action_dim, env, epochs, batch_size, clip_epsilon, value_coeff, entropy_coeff, clip_epsilon, learning_rate , gamma, lamda)
