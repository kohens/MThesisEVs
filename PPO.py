...
def av(Array, time1, time2):
    if time1 >= time2:
        return Array[time2]
    else: return np.mean(Array[time1:time2])
...
def get_EV_fleet(t, IDs, E_req, P_lim, T_arr, T_dep, Stations, StationVector, L_c): 
    ...
    IDs = np.multiply(vec, IDs)[vec != 0]
    T_arr = np.multiply(vec, T_arr)[vec != 0]
    T_dep = np.multiply(vec, T_dep)[vec != 0]
    P_lim = np.multiply(vec, P_lim)[vec != 0]
    E_req = np.multiply(vec, E_req)[vec != 0]
    Stations = np.multiply(vec, Stations)[vec != 0]
    IDvec = np.zeros( len(StationVector))
    T_arrvec =  np.zeros( len(StationVector))
    T_depvec = np.zeros( len(StationVector))
    P_limvec = np.zeros( len(StationVector))
    E_reqvec = np.zeros( len(StationVector))
    for i in range(len(Stations)):
      j = np.where(StationVector == Stations[i]*np.ones(len(StationVector)), np.ones(len(StationVector)), np.zeros(len(StationVector)))
      IDvec += j*IDs[i]
      T_arrvec += j*T_arr[i]
      T_depvec += j*T_dep[i]
      P_limvec += j*P_lim[i]
...
def heuristic_dispatch(P, P_min, P_max, delta_T_dep, E_req, P_lim, N_ev):
    ...
    P_b = np.zeros( N_ev)
    #first ensure that all vehicles get their minimum required eneryg
    for i in range(N_ev):
        index = sorted_indices[i]
        P_b[index] = min(P_max[index], min(P, P_min[index]))
        P -= P_b[index]
    #further heuristic dispatch of remaining power
    times = 0
    while np.sum(P_b) < np.sum(E_req) and P > 0:
        for i in range(N_ev):
            index = sorted_indices[i]
            Extra_power = min(P_max[index]-P_b[index], P)
            P -= Extra_power
            P_b[index] += Extra_power
    return P_b
...
def real_time_controller(P, M_start, M_end, L_c, N_ev, P_lim, t, k, pv_generation):
    EVs = [i for i in range(len(P)) if P[i] > 0]
    P_r = np.zeros((N_ev, L_c))
    
    P_r_max = np.zeros( len(P))
    P_r_min = np.zeros( len(P))
    E_rem = np.zeros( len(P))
    for i in EVs:
        P_r_max[i] = min(P_lim[i], P[i]*k)
        P_r_min[i] = P[i]/k
        E_rem[i] = P[i]*(M_end[i]-M_start[i])/60
...
class Environment(gym.Env):
    ...
    def get_past_PV(self):
        P_pv_past = np.zeros(self.N_past)
        for j in range(self.N_past):
            index1 = -(j+1)*60//self.L_c-1
            index2 = -(j)*60//self.L_c-1
            P_pv_past[j] = av(self.P_pv_past, index1, index2)
        return P_pv_past
    ...
    def get_forecast_tuple(self):
        P_f_past = np.zeros( self.N_past)
        P_f_fut = np.zeros( self.N_fut)
        P_f_rest = np.zeros(1)
        for j in range(self.N_past):
            index1 = (self.t-self.start_time)*60/self.L_c-(j+1)*60/self.L_c
            index2 = (self.t-self.start_time)*60/self.L_c-(j)*60/self.L_c
            index1 = max(index1, 0)
            index2 = max(index2, 0)
            end = len(self.pv_forecast)-1
            index1 = min(index1, end)
            index2 = min(index2, end)
            P_f_past[j] = av(self.pv_forecast, int(index1), int(index2))
        for j in range(self.N_fut):
            index1 = (self.t-self.start_time)*60/self.L_c+(j)*60/self.L_c
            index2 = (self.t-self.start_time)*60/self.L_c+(j+1)*60/self.L_c
            index1 = int(max(index1, 0))
            index2 = int(max(index2, 0))
            end = len(self.pv_forecast)-1
            index1 = min(index1, end)
            index2 = min(index2, end)
            P_f_fut[j] = av(self.pv_forecast,  int(index1), int(index2))
        index =(self.t-self.start_time)*60/self.L_c+self.N_fut*60/self.L_c+1
        P_f_rest[0] = av(self.pv_forecast,int(index), len(self.pv_forecast)-1)
        
        return np.concatenate((P_f_past, P_f_fut, P_f_rest))
    ...
    def get_price_tuple(self):
        L_t = np.zeros(1)
        L_f_rest = np.zeros(1)
        L_t[0]= self.consumption_price(self.t)
        L_f_fut = np.zeros( self.N_fut)
        for j in range(self.N_fut):
            index1 = (self.t-self.start_time)*60/self.L_c+(j)*60/self.L_c
            index2 = (self.t-self.start_time)*60/self.L_c+(j+1)*60/self.L_c
            index1 = int(max(index1, 0))
            ...
            index1 = min(index1, end)
            index2 = min(index2, end)
            L_f_fut[j] = av(self.price_forecast, int(index1), int(index2))
        index = int((self.t+self.N_fut-self.start_time)*60/self.L_c)+1
        L_f_rest[0]= av(self.price_forecast, index, len(self.price_forecast)-1)
        return np.concatenate((L_t, L_f_fut, L_f_rest))
    ...
    def calculate_reward(self, P_charge):
        ...
        for i in range(self.L_c):
            P_diff = sum(P_charge[:, i]) - self.pv_generation(self.t+i/60)
            if P_diff >= 0:
                cost += P_diff*self.consumption_price(self.t+i/60)/60
            else:
                cost += P_diff*self.injection_price(self.t+i/60)/60
        return -1*cost
    ...
    def calculate_advantages_and_targets(self, rewards, next_states, dones):
        ...
        for i in reversed(range(len(rewards))):
            if i == len(rewards)-1:
                ...
                next_val = denormalized_values[i+1]
            delta = rewards[i] + self.discount_factor * next_val * (1 - dones[i]) - values[i]
            gae = delta + self.discount_factor * self.lamda * (1 - dones[i]) * gae
            advantages.insert(0, gae)
            target_values.insert(0, rewards[i] + self.discount_factor * (1 - dones[i]) * next_val)
